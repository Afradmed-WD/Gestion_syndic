...
const STATS = [
  { icon: "fa-building", bg: "bg-indigo-500", label: "Total appartements", value: "156", note: "100% du total", tone: "text-slate-400" },
  { icon: "fa-house-user", bg: "bg-emerald-500", label: "Occupés", value: "132", note: "84.6% du total", tone: "text-emerald-500" },
  { icon: "fa-key", bg: "bg-amber-500", label: "Vacants", value: "18", note: "11.5% du total", tone: "text-amber-500" },
  { icon: "fa-wrench", bg: "bg-rose-500", label: "En maintenance", value: "6", note: "3.8% du total", tone: "text-rose-500" },
];

const APARTMENTS = [
  { num: "A12", floor: "1", surface: 120, owner: "Ahmed Benali", status: "Occupé", date: "12 Jan 2020" },
  { num: "B05", floor: "1", surface: 95, owner: "Fatima Zahra", status: "Occupé", date: "18 Fév 2021" },
  { num: "C03", floor: "2", surface: 110, owner: "Youssef Omar", status: "Occupé", date: "25 Mar 2019" },
  { num: "D15", floor: "2", surface: 100, owner: "Maria Amine", status: "Occupé", date: "30 Avr 2022" },
  { num: "E07", floor: "3", surface: 130, owner: "Sami Ali", status: "Vacant", date: "–" },
  { num: "F02", floor: "3", surface: 90, owner: "Khadija Hamid", status: "Occupé", date: "10 Mai 2021" },
  { num: "G11", floor: "4", surface: 105, owner: "Mohamed Ilyas", status: "En maintenance", date: "–" },
  { num: "A01", floor: "4", surface: 80, owner: "Amina Zaher", status: "Occupé", date: "15 Jui 2020" },
].map((a, i) => ({ ...a, initials: initialsOf(a.owner), color: AVATAR_COLORS[i % AVATAR_COLORS.length] }));
...
function Appartements() {
  ...
  try {
    ...
    const filtered = APARTMENTS.filter(
      (a) => a.num.toLowerCase().includes(search.toLowerCase()) || a.owner.toLowerCase().includes(search.toLowerCase())
    );
          ...
          <div className="p-8">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100 text-slate-400 text-left">
                    ...
                    <th className="font-medium">Date d'acquisition</th>
                    <th className="font-medium text-right pr-6">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((a, i) => (
                    <tr key={a.num} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/60 transition-colors">
                      <td className="py-4 px-6 text-slate-400">{i + 1}</td>
                      <td className="font-semibold text-slate-700">{a.num}</td>
                      <td className="text-slate-500">{a.floor}</td>
                      <td className="text-slate-500">{a.surface}</td>
                      <td>
                        <div className="flex items-center gap-3">
                          <span className={`w-9 h-9 rounded-full ${a.color.bg} ${a.color.text} flex items-center justify-center text-xs font-bold shrink-0`}>
                            {a.initials}
                          </span>
                          <span className="font-semibold text-slate-700 whitespace-nowrap">{a.owner}</span>
                        </div>
                      </td>
                      <td>
                        <StatusBadge status={a.status} />
                      </td>
                      <td className="text-slate-500 whitespace-nowrap">{a.date}</td>
                      <td className="pr-6">
            ...
            <div className="flex items-center justify-between mt-5 flex-wrap gap-3">
              <p className="text-sm text-slate-400">
                Affichage de 1 à {filtered.length} sur 156 résultats
              </p>
