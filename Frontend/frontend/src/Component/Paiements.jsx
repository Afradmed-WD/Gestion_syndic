...
const STATS = [
  { icon: "fa-dollar-sign", bg: "bg-emerald-500", label: "Total paiements", value: "125 680", suffix: "MAD", note: "↑ 18% ce mois", tone: "text-emerald-500" },
  { icon: "fa-credit-card", bg: "bg-blue-500", label: "Payés", value: "98 450", suffix: "MAD", note: "78.3% du total", tone: "text-blue-500" },
  { icon: "fa-clock", bg: "bg-amber-500", label: "En attente", value: "18 950", suffix: "MAD", note: "15.1% du total", tone: "text-amber-500" },
  { icon: "fa-times-circle", bg: "bg-rose-500", label: "En retard", value: "8 280", suffix: "MAD", note: "6.6% du total", tone: "text-rose-500" },
];

const PAYMENTS = [
  { date: "12/05/2024", resident: "Ahmed Benali", apt: "A12", amount: "1 200", mode: "Virement", status: "Payé" },
  { date: "11/05/2024", resident: "Fatima Zahra", apt: "B05", amount: "950", mode: "Espèces", status: "En attente" },
  { date: "10/05/2024", resident: "Youssef Omar", apt: "C03", amount: "1 100", mode: "Virement", status: "Payé" },
  { date: "09/05/2024", resident: "Maria Amine", apt: "D15", amount: "1 300", mode: "Chèque", status: "Payé" },
  { date: "08/05/2024", resident: "Sami Ali", apt: "E07", amount: "800", mode: "Virement", status: "En attente" },
  { date: "07/05/2024", resident: "Khadija Hamid", apt: "F02", amount: "1 050", mode: "Espèces", status: "Payé" },
  { date: "06/05/2024", resident: "Mohamed Ilyas", apt: "G11", amount: "1 200", mode: "Virement", status: "Payé" },
  { date: "05/05/2024", resident: "Amina Zaher", apt: "A01", amount: "900", mode: "Espèces", status: "En attente" },
].map((p, i) => ({ ...p, initials: initialsOf(p.resident), color: AVATAR_COLORS[i % AVATAR_COLORS.length] }));
...
function Paiements() {
  ...
  try {
    ...
    const filtered = PAYMENTS.filter(
      (p) => p.resident.toLowerCase().includes(search.toLowerCase()) || p.apt.toLowerCase().includes(search.toLowerCase())
    );
          ...
          <div className="p-8">
                ...
                <tbody>
                  {filtered.map((p, i) => (
                    <tr key={i} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="text-slate-500 whitespace-nowrap">{p.date}</td>
                      <td>
                        <div className="flex items-center gap-3">
                          <span className={`w-9 h-9 rounded-full ${p.color.bg} ${p.color.text} flex items-center justify-center text-xs font-bold shrink-0`}>
                            {p.initials}
                          </span>
                          <span className="font-semibold text-slate-700 whitespace-nowrap">{p.resident}</span>
                        </div>
                      </td>
                      <td className="text-slate-500">{p.apt}</td>
                      <td className="text-slate-700 font-semibold">{p.amount}</td>
            ...
            <div className="flex items-center justify-between mt-5 flex-wrap gap-3">
              <p className="text-sm text-slate-400">
                Affichage de 1 à {filtered.length} sur 45 résultats
              </p>
